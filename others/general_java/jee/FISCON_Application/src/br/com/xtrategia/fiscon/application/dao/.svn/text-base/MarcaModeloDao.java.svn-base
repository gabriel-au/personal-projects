...
public class MarcaModeloDao extends ConexaoBanco<MarcaModeloPojo> {

	@Override
	public void iniciar() throws SQLException {
		//Inativar todos os registros
		LogApplicacao.gravar("Processo de Inativar os registros");
		String sql="update marca_modelo set ativo= false where ativo=true";
		Statement stmt = conn.createStatement();
		stmt.execute(sql);
	}
	
	@Override
	public void executar(MarcaModeloPojo pojo)throws SQLException {
		//consultar se existe
		pstmConsulta.setString(1, pojo.getCodigo());
		ResultSet rs = pstmConsulta.executeQuery();
		//se existir
		if(rs.next()){
			MarcaModeloPojo pojoOld = new MarcaModeloPojo();
	...
	public String getSqlConsulta() {
		return "Select id,ativo,data_modificacao,codigo,nome from marca_modelo where codigo = ?";
	}

	@Override
	public String getSqlInsert() {
		return "insert into marca_modelo (ativo,data_modificacao,codigo,nome)"
				+ "values (?,?,?,?)";
	...
	public String getSqlInsertHistorico() {
		return "insert into historico_marca_modelo (id,ativo,data_modificacao,codigo,nome)"
				+ "values (?,?,?,?,?)";
	}

	@Override
	public String getSqlUpdate() {
		return "update marca_modelo set " +
