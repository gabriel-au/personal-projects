...
-(void)bluetoothDiscoverComplete:(BOOL)success
{
    ...
    [prn btSetEnabled:false error:nil];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL r=[prn btSetEnabled:true error:&error];
    if(r)
        r=[prn btConnect:[btAddresses objectAtIndex:indexPath.row] pin:@"0000" error:&error];
    [progressViewController.view removeFromSuperview];
    if(r)
    {
    ...
    {
        //connection failed, disable the module to save battery
        [prn btSetEnabled:false error:nil];
...
-(IBAction)onBTDiscover:(id)sender
{
    ...
    NSError *error;
    [prn btSetEnabled:false error:nil];
    if(![prn btSetEnabled:true error:&error] || ![prn btDiscoverPrintersInBackground:&error])
    {
