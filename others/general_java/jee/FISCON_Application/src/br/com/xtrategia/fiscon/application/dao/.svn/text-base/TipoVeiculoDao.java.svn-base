...
public class TipoVeiculoDao extends ConexaoBanco<TipoVeiculoPojo> {

	@Override
	public void iniciar() throws SQLException {
		//Inativar todos os registros
		LogApplicacao.gravar("Processo de Inativar os registros");
		String sql="update tipo set ativo= false where ativo=true";
		Statement stmt = conn.createStatement();
		stmt.execute(sql);
	}
	
	@Override
	public void executar(TipoVeiculoPojo pojo)throws SQLException {
		//consultar se existe
		pstmConsulta.setString(1, pojo.getCodigo());
		ResultSet rs = pstmConsulta.executeQuery();
		//se existir
		if(rs.next()){
			TipoVeiculoPojo pojoOld = new TipoVeiculoPojo();
	...
	public String getSqlConsulta() {
		return "Select id,ativo,data_modificacao,codigo,nome from tipo where codigo = ?";
	}

	@Override
	public String getSqlInsert() {
		return "insert into tipo (ativo,data_modificacao,codigo,nome)"
				+ "values (?,?,?,?)";
	...
	public String getSqlInsertHistorico() {
		return "insert into historico_tipo (id,ativo,data_modificacao,codigo,nome)"
				+ "values (?,?,?,?,?)";
	}
