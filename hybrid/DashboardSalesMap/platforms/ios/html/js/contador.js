$().ready(function() {
	...
	function vender() {
		...
		filtro();
		alertaVenda(estadoNome[randEstado+1]);
	...
	function alertaVenda(estado) {
		...
		new geocoder.geocode(address, function(results, status) {
			marker = new google.maps.Marker({
				...
				icon:'img/Icon_Venda.png',
				clickable: false,
				map: map
			  });
			
			/*venda = new google.maps.Circle({
		      strokeColor: "#225e9a",
		      strokeOpacity: 1,
		      strokeWeight: 1,
		      fillColor: "#f47529",
		...
		//console.log('#addressZoom lat: '+ latLngEstado.lat() + ' long: ' + latLngEstado.lng());
	...
	function filtro() {
		...
		if ($("input[name=pais][value=br]").is(":checked")) {
			...
			$("input[name=estado]:checked").each(function()	{
				...
				//console.log('variavel: ' + vendaEstado[(parseInt($(this).attr("value")))-1]);
			});
		}
		...
		//somaFiltro = parseInt((somaFiltro/7)*canalCount);
		
		//somaFiltro = 150000;
		
		//somaFiltro = parseInt((somaFiltro/7)*produtoCount);
		
		//console.log('somaFiltro: ' + somaFiltro);
		//console.log('somaBrasil: ' + soma);
