 ...
 *
 * Custom version by Leo Dutra (twitter.com/leodutra)
 * Provides horizontal scrolling without a definition of pages.
 * Some base css properties are required... like on the original version.
 */
...
var m = Math,
	...
	TWIS = function (wrapper, options) {
		...
		if (that.options.useTransform) that.slider.style[vendor + 'Transform'] = trnOpen + that.x + 'px,' + that.y + 'px' + trnClose;
...
TWIS.prototype = {
	...
	_pos: function (x, y) {
		if (this.options.useTransform) {
				this.slider.style[vendor + 'Transform'] = trnOpen + x + 'px,'+ y + 'px'+ trnClose;
	...
	_start: function (e) {
		...
		if (that.options.momentum) {
			if (that.options.useTransform) {
				...
				y = getComputedStyle(that.slider, null).top.replace(/[^0-9-]/g, '') * 1;
			}
			
			if (x != that.x || y != that.y) {
				//if (that.options.useTransition) that._unbind('webkitTransitionEnd', that.scroller);
				if (that.options.useTransition) that._unbind('webkitTransitionEnd', that.slider);
		...
		that.startX = that.absStartX = that.x;	// Needed by snap threshold
		that.startY = that.absStartY = that.y;
...
//		}
		...
		if (that.dirX && duration < 300 && that.options.momentum) {
			momentumX = newPosX ? that._momentum(newPosX - that.startX, duration, -that.x, (that.maxScrollX < 0 ? that.scrollerW - that.wrapperW + that.x : 0), that.options.bounce ? that.wrapperW : 0) : momentumX;
		...
		if (momentumX.dist) {
			newDuration = m.max(momentumX.time, 10);

			that.scrollTo(m.round(newPosX), that.y, newDuration);
	...
	_transitionEnd: function (e) {
		...
		that._unbind('webkitTransitionEnd', that.slider);
	...
	_startAni: function () {
		...
		if (that.options.useTransition) {
			...
			if (step.time) that._bind('webkitTransitionEnd', that.slider);
...
//	},
	...
	destroy: function () {
		...
		that.slider.style[vendor + 'Transform'] = '';
...
//		that.pageCount = that.pages.length;
		...
		that.scrollerH = that.slider.offsetHeight;
...
//	},
	...
	scrollX: function (pos, time) {
		time = time || 0;
		if (pos > 0) pos = 0;
		else if (pos < this.maxScrollX) pos = this.maxScrollX;

		this.scrollTo(pos, this.y, time);
	},
