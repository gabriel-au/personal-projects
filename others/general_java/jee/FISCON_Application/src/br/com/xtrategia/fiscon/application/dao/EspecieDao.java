...
public class EspecieDao extends ConexaoBanco<EspeciePojo> {

	@Override
	public void iniciar() throws SQLException {
		//Inativar todos os registros
		LogApplicacao.gravar("Processo de Inativar os registros");
		String sql="update especie set ativo= false where ativo=true";
		Statement stmt = conn.createStatement();
		stmt.execute(sql);
	}
	
	@Override
	public void executar(EspeciePojo pojo)throws SQLException {
		//consultar se existe
		pstmConsulta.setString(1, pojo.getCodigo());
		ResultSet rs = pstmConsulta.executeQuery();
		//se existir
		if(rs.next()){
			EspeciePojo pojoOld = new EspeciePojo();
	...
	public String getSqlConsulta() {
		return "Select id,ativo,data_modificacao,codigo,nome from especie where codigo = ?";
	}

	@Override
	public String getSqlInsert() {
		return "insert into especie (ativo,data_modificacao,codigo,nome)"
				+ "values (?,?,?,?)";
	...
	public String getSqlInsertHistorico() {
		return "insert into historico_especie (id,ativo,data_modificacao,codigo,nome)"
				+ "values (?,?,?,?,?)";
	}

	@Override
	public String getSqlUpdate() {
		return "update especie set " +
