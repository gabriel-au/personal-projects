...
-(void)setAuthKeyAndFlags:(uint32_t)flags
{
    ...
    if([linea cryptoSetKey:KEY_AUTHENTICATION key:newKeyData oldKey:oldKeyData keyVersion:[newAuthenticationKeyVersion.text intValue] keyFlags:flags error:&error])
    {
        [self displayAlert:NSLocalizedString(@"Operation successful!",nil) message:NSLocalizedString(@"Key successfully set",nil)];
        //setting key okay, modify the authentication program uses so we can easily authenticate with linea
...
-(IBAction)deleteAuthenticationKey:(id)sender
{
	...
	if([linea cryptoSetKey:KEY_AUTHENTICATION key:newKeyData oldKey:oldKeyData keyVersion:0 keyFlags:0 error:&error])
    {
...
-(IBAction)setEncryptionKey:(id)sender
{
	...
	if([linea cryptoSetKey:KEY_ENCRYPTION key:newKeyData oldKey:oldKeyData keyVersion:[newEncryptionKeyVersion.text intValue] keyFlags:0 error:&error])
    {
...
-(IBAction)deleteEncryptionKey:(id)sender
{
	...
	if([linea cryptoSetKey:KEY_ENCRYPTION key:newKeyData oldKey:oldKeyData keyVersion:0 keyFlags:0 error:&error])
    {
        [self displayAlert:NSLocalizedString(@"Operation successful!",nil) message:NSLocalizedString(@"Key successfully deleted",nil)];
    }else
        ERRMSG(NSLocalizedString(@"Operation failed!",nil));
...
-(IBAction)checkAuthentication:(id)sender
{
	...
	if([linea cryptoAuthenticateLinea:lockedKeyData error:&error])
    {		
		[self displayAlert:@"Linea Pro" message:NSLocalizedString(@"Correct Device - UnLocked...",nil)];
	}
	else
    {
		[self displayAlert:@"Linea Pro" message:NSLocalizedString(@"Wrong Device - Locked...",nil)];
	}
}
...
-(IBAction)unlockLinea:(id)sender
{
	...
	NSData *lockedKeyData=[authKey.text dataUsingEncoding:NSASCIIStringEncoding];
	if([linea cryptoAuthenticateiPod:lockedKeyData error:&error])
    {		
		[self displayAlert:@"Linea Pro" message:NSLocalizedString(@"Unlock successful",nil)];
	}else
    {
		[self displayAlert:@"Linea Pro" message:NSLocalizedString(@"Unlock failed",nil)];
	}
}
...
-(IBAction)getKeyInfo:(id)sender
{
    ...
    if([linea cryptoGetKeyVersion:KEY_ENCRYPTION keyVersion:&encKeyVersion error:&error] && [linea cryptoGetKeyVersion:KEY_AUTHENTICATION keyVersion:&authKeyVersion error:&error])
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    NSLog(@"key: %@, len=%d",decryptKey.text,[decryptKey.text length]);
