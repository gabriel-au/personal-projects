 ...
 * 
 */
public class HomologarQuantidadeItensAutoInfracaoCommand extends
		AbstractCommand {
	...
	public TransferObject execute(TransferObject transferObject)
		...
		try {
			...
			Criteria criteria = sessao
					.createCriteria(transferObject.getClass())
			...
			if (pojo.getVeiculoPojo() != null
				...
				Criteria cri = sessao.createCriteria(VeiculoPojo.class).add(
						Restrictions.eq("veiculoPlaca", pojo.getVeiculoPojo()
								.getVeiculoPlaca().toUpperCase()));
			...
			if (pojo.getNumeroAutoInfracao() != null
				...
				criteria.add(Restrictions.eq("numeroAutoInfracao", pojo
