	...
	else
	{
...
#endif
		{
			...
			if(error)
			{
				if(errPtr)
				{
					NSString *errMsg = [NSString stringWithCString:gai_strerror(error) encoding:NSUTF8StringEncoding];
...
- (NSError *)errorFromCFStreamError:(CFStreamError)err
{
	...
	else if(err.domain == kCFStreamErrorDomainNetDB)
	{
		...
		message = [NSString stringWithCString:gai_strerror(err.error) encoding:NSUTF8StringEncoding];
	}
...
- (NSString *)addressHost:(CFDataRef)cfaddr
{
	...
	return [NSString stringWithCString:pStr encoding:NSUTF8StringEncoding];
}
