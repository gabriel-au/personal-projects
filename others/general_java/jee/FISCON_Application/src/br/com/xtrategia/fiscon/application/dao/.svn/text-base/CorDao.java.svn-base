...
public class CorDao extends ConexaoBanco<CorPojo> {

	@Override
	public void iniciar() throws SQLException {
		//Inativar todos os registros
		LogApplicacao.gravar("Processo de Inativar os registros");
		String sql="update cor set ativo= false where ativo=true";
		Statement stmt = conn.createStatement();
		stmt.execute(sql);
	}
	
	@Override
	public void executar(CorPojo pojo)throws SQLException {
		//consultar se existe
		pstmConsulta.setString(1, pojo.getCodigo());
		ResultSet rs = pstmConsulta.executeQuery();
		//se existir
		if(rs.next()){
			CorPojo pojoOld = new CorPojo();
	...
	public String getSqlConsulta() {
		return "Select id,ativo,data_modificacao,codigo,nome from cor where codigo = ?";
	}

	@Override
	public String getSqlInsert() {
		return "insert into cor (ativo,data_modificacao,codigo,nome)"
				+ "values (?,?,?,?)";
	...
	public String getSqlInsertHistorico() {
		return "insert into historico_cor (id,ativo,data_modificacao,codigo,nome)"
				+ "values (?,?,?,?,?)";
	}

	@Override
	public String getSqlUpdate() {
		return "update cor set " +
