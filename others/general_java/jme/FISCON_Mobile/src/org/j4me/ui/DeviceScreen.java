...
public abstract class DeviceScreen
{
	...
	public int getTitleHeight() {
		return UIManager.getTheme().getTitleHeight();
	}
}
	...
	implements CommandListener
{
	...
	protected void pointerPressed (int x, int y)
	{
		...
		{
			int titleHeight;
			if(master.getTitleHeight()>0){
				titleHeight = master.getTitleHeight();//theme.getTitleHeight();
			}else{
		...
		if ( processed == false )
		{
			...
			if ( hasTitle )
			{
				...
				if(master.getTitleHeight()>0){
					py -= master.getTitleHeight();//theme.getTitleHeight();
				}else{
	...
	protected void pointerDragged (int x, int y)
	{
		...
		if ( master.hasTitleBar() )
		{
			...
			if(master.getTitleHeight()>0){
				...
				py -= UIManager.getTheme().getTitleHeight();
			}
		}
